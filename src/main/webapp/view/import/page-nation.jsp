	...
	<nav class="datatable-pagination">
		<ul class="datatable-pagination-list">
			...
			<c:if
					test="${ pageMaker.prev }">
					<a
						href="${ pageContext.servletContext.contextPath }/main?pageNo=${ pageMaker.startPage -1}"
			...
			<c:forEach begin="${ pageMaker.startPage }"
				...
				<c:choose>
					<c:when test="${ status.index == pageMaker.cri.pageNum }">
						<c:set scope="page" var="selectedBgColor"
							value="style='background-color:aqua;'"></c:set>
					</c:when>
					<c:otherwise>
				...
				<li class="datatable-pagination-list-item"><a
					href="${pageContext.servletContext.contextPath}/main?pageNo=${status.index}"
			...
			<li class="datatable-pagination-list-item"><c:if
					test="${ pageMaker.next }">
					<a
						href="${pageContext.servletContext.contextPath}/main?pageNo=${pageMaker.endPage + 1}"
						class="datatable-pagination-list-item-link">›</a>
				</c:if></li>
		</ul>
	</nav>
