...
<head>
  ...
  <script>
     ...
     function goSubmit(){
         ...
         $.ajax(
                 {
                  url: "/project/api/board" // contextpath
                 ,async:true  // 비동기 쓰레드, false ; 동기식 (응답 받을 때 까지 대기)
                 ,contentType:'application/x-www-form-urlencoded; chaset=UTF-8' // 전송타입
                 ,type:"POST" // method or type
                 ,data:data
                 ,dataType:"json"  // 수신타입
                 ,success:function(data, textStatus){
                 	...
                 	if(data.result == 1){
                 		switch(data.method){
                 		case'modify': msg = "수정되었습니다." break;
                 		case 'register' : msg = "뿌애앵애애앵"; break;
                 		}
                 		alert(msg);
                 	}else{
                 		alert(data.result + '다시 시도하세요'.);
                 	}
                  }
  ...
  <form action="/action_page.php" name = "form" method = "post">
    ...
    <div class="row">
        ...
        <div class="mb-3 mt-3 col p-3">
            ...
            <div class="form-control" id="id" name="id">${ requestScope.boardVO.id == null ? sessionScope.SESSID : requestScope.boardVO.id }</div>
        </div> 
    </div>
    <div class="mb-3 mt-3">
    ...
    <div class="row">
       ...
       <button type="submit" class="register col p-3 btn btn-primary" onclick="">Submit</button> <!-- style="display: ${ param.method=='get' ? 'none' : ''}"  -->
       <div class="col p-3"></div>
       <button type="button"  class="remove col p-3 btn btn-primary" onclick="adoptMethod('remove');">Remove</button>
       <div class="col p-3"></div>
    ...
    <input type="hidden" name="id" value="${boardVO.id == null ? sessionScope.SEES_ID : requestScope.boardVO.id}">
  </form>
</div>
