...
<%
								String reasd = Integer.toString(list.get(num).getStay_no());
								%>

								<%
								//int foodNo = 1; // replace with the correct foodNo value
								StayReviewDAO reviewDAO = StayReviewDAOImpl.getInstance();
								List<StayReviewVO> reviewList = reviewDAO.selectStayReview(list.get(num).getStay_no());
								%>
								...
								<div>
									...
									<form action="<%=request.getContextPath()%>/StayReview"
										method="post">
										<input type="hidden" name="stayNo" value="<%=reasd%>" /> <label
								...
								<div>
									...
									<ul>
										<%
										for (StayReviewVO review : reviewList) {
										%>
										<li>
											<div>
												작성자:
												...
												<%
												String stay_review_user_id = review.getUser_id();
												String current_user_id = (String) session.getAttribute("SESS_ID");
												if (stay_review_user_id.equals(current_user_id)) {
												%>
												<form
													action="<%=request.getContextPath()%>/DeleteStayReview"
													method="post">
