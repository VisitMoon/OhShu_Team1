				...
				<div class="container-fluid px-4">
					...
					<div class="card mb-4">
						...
						<% FoodService service = FoodServiceImpl.getInstance();  
							List<FoodVO> food_list = service.getFoodList();
							String foodNo = request.getParameter("foodNo");
							int num = Integer.parseInt(foodNo);
							%>
							<p>
								<a>전체</a><a> > </a><a><%=food_list.get(num).getFood_source()%></a><a>
									> </a> <a><%=food_list.get(num).getFood_name()%></a>
							</p>
						</div>
						<div class="card-body">
							<div class="main">
								...
								<div style="text-align: center;">
									<img src="<%=food_list.get(num).getFood_img()%>"
										style="border-radius: 20px;">
								</div>
								
								<h1 style="font-weight: bold;">상세내용</h1>
								<div><%=food_list.get(num).getFood_info()%></div>
								...
								<%=food_list.get(num).getFood_x()%>
									,
								<%=food_list.get(num).getFood_y()%>
									), // 서울의 좌표
										zoom : 10
									// 초기 줌 레벨 설정
									};
								...
								<div style="margin-top: 60px;">
									...
									<table>
										...
										<tr>
											<td><%=food_list.get(num).getFood_name()%></td>
											<td><%=food_list.get(num).getFood_location()%></td>
											<td><%=food_list.get(num).getFood_address()%></td>
											<td><%=food_list.get(num).getFood_tel()%></td>
